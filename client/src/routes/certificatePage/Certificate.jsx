...
const fetchEventName = async (eventId, eventType) => {
  try {
    ...
    if (eventType === 'MINICOURSE') {
      url = `https://cd4a-2804-29b8-500a-1274-e81e-e223-a60-a299.ngrok-free.app//minicourses/${eventId}`;
    } else if (eventType === 'LECTURE') {
      url = `https://cd4a-2804-29b8-500a-1274-e81e-e223-a60-a299.ngrok-free.app//lectures/${eventId}`;
    } else {
...
const Certificates = () => {
  ...
  useEffect(() => {
    const fetchCertificates = async () => {
      try {
        const response = await axios.get('https://cd4a-2804-29b8-500a-1274-e81e-e223-a60-a299.ngrok-free.app//users/certificates/', { withCredentials: true });
  ...
  const handleRemoveCertificate = async (certificateId) => {
    try {
      await axios.delete(`https://cd4a-2804-29b8-500a-1274-e81e-e223-a60-a299.ngrok-free.app//certificates/${certificateId}`, { withCredentials: true });
              ...
              certificates.map((cert) => (
                <div key={cert.certificateId} className="certificate-card">
                  ...
                  <div className="buttons-certificate">
                    <a href={`https://cd4a-2804-29b8-500a-1274-e81e-e223-a60-a299.ngrok-free.app//certificates/issue/${cert.certificateId}`} target="_blank" rel="noopener noreferrer">
